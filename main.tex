...
an airport scenario comprising three aircraft sharing one runway, two bays, and
five ground service worker pools. The model is analysed using the TINA toolbox to
...
arising from circular worker pool competition and resolve them via a
\textit{GSE\_Done} control place. All defined safety and liveness properties are verified in the
corrected model.
\end{abstract}
...
and Docking}, where the aircraft lands, taxis to its assigned bay, and passengers
deplane; (2)~\textit{Ground Handling}, where multiple service teams work in
parallel to prepare the aircraft for its next flight; and (3)~\textit{Departure
Preparation}, where passengers board and the aircraft pushes back for takeoff.

The ground handling phase is particularly challenging because it involves
concurrent operations by multiple teams sharing limited resources such as
apron space and ground support equipment (GSE)~\cite{b1,b2}. Simulation-based
approaches have been widely used to analyse these operations, but they cannot
provide formal guarantees about deadlock-freedom or safety property correctness
under all possible resource contention scenarios. When multiple concurrent
processes compete for shared resources, the risk of deadlock arises: a situation
where every process is waiting for a resource held by another, resulting in a
complete standstill. For example, a fuel truck may block the path of a catering
truck, which in turn blocks a baggage cart, which blocks the fuel truck, creating
...
through three operational phases: (1)~\textit{Arrival}, covering scheduling,
landing, taxi, deplaning, and docking; (2)~\textit{Ground Handling}, comprising
five concurrent service tasks; and (3)~\textit{Departure}, consisting of
boarding, pushback, and takeoff.
...
\begin{figure}[t]
  ...
  \caption{Petri Net model of the three-aircraft turnaround process with shared
           worker pools and fast/slow service paths. Apron resources structurally
           limit concurrent ground handling to two aircraft at a time.}
...
two task transitions are defined per service: a \textit{fast} transition
(prefix \texttt{sf\_}) that consumes the majority of $N^{\delta}$ workers and
fires within $[\mathit{EFT}_f, \mathit{LFT}_f]$, and a \textit{slow} transition
(prefix \texttt{ss\_}) that consumes only $W_{\min}$ workers and fires within
$[\mathit{EFT}_s, \mathit{LFT}_s] > [\mathit{EFT}_f, \mathit{LFT}_f]$.
In the current model, A1 is assigned the fast path for all services
(e.g., $\mathit{sf\_maintenance}$ consuming 10 workers), while A2 and A3
are assigned the slow path (e.g., $\mathit{ss\_maintenance2}$ and
$\mathit{ss\_maintenane3}$ each consuming 2 workers).
...
Table~\ref{tab:worker_params} presents the worker pool parameters for each ground
service. To keep the state space tractable for TINA, firing intervals are
assigned only to the completion transitions rather than to every individual
start and activity transition. All fast completion transitions share the
interval $[3,~5]$, and all slow completion transitions share $[6,~9]$,
reflecting the difference in task duration between full-crew and minimum-crew
deployments. Pool sizes and crew allocations are informed by operational
benchmarks from the literature~\cite{b1,b2,b3}.

\begin{table}[h!]
  \caption{Worker Pool Parameters Per Ground Service}
  \label{tab:worker_params}
  \footnotesize
  \begin{tabularx}{\columnwidth}{@{}Xcccc@{}}
    \toprule
    \textbf{Service} & \textbf{Pool ($N$)} & \textbf{Fast crew} & \textbf{Slow crew} & \textbf{Interval (fast/slow)} \\
    \midrule
    Unloading   & 6  & 6  & 2 & {[}3,5{]}/{[}6,9{]} \\
    Cleaning    & 8  & 6  & 2 & {[}3,5{]}/{[}6,9{]} \\
    Fueling     & 4  & 3  & 1 & {[}3,5{]}/{[}6,9{]} \\
    Catering    & 8  & 6  & 2 & {[}3,5{]}/{[}6,9{]} \\
    Maintenance & 10 & 10 & 2 & {[}3,5{]}/{[}6,9{]} \\
    \bottomrule
  \end{tabularx}
\end{table}
...
from the same worker pool. Consider the \textit{maintenance\_team} place
($N = 10$): if A1 fires $\mathit{sf\_maintenance}$ (consuming all 10 workers),
the pool is exhausted and A2 cannot fire $\mathit{sf\_maintenance}$. A2 may
still fire $\mathit{ss\_maintenance2}$ (consuming 2 workers) if sufficient
tokens remain; otherwise it must wait until A1 completes and returns the workers
via $\mathit{ef\_maintenance}$. The arc weights therefore create a natural
priority ordering without the need for inhibitor arcs, preserving decidability.
Note that because $\mathit{appron\_resource}$ limits concurrent ground handling
to two aircraft, a third aircraft (A3) cannot enter this contention scenario
until one of the active aircraft departs.

A deadlock condition emerges when circular waiting occurs across multiple pools.
For example: A1 holds \textit{fuel\_team} and waits for \textit{catering\_team},
while A2 holds \textit{catering\_team} and waits for \textit{fuel\_team}. This
is the classic circular wait condition, and it is detected by TINA as a dead
marking in the reachability graph. We introduce a \textit{GSE\_Done} control
place with an initial marking of 2 to bound the total number of concurrent GSE
operations; each completion transition (\textit{all\_done\_1},
\textit{all\_done2}, \textit{all\_done3}) returns a token to \textit{GSE\_Done},
which is consumed again only when a new \textit{board} transition fires. This
resolves the circular wait following the deadlock avoidance and liveness control
approaches established in~\cite{b4,b5}.

\subsection{Safety Constraints in the Model}

Safety constraints are encoded as structural preconditions. Pushback requires a
token in $\mathit{d\_fuel}$, ensuring refueling is complete before the aircraft
moves. Boarding requires tokens in all five completion places
($\mathit{d\_fuel}$, $\mathit{d\_catering}$, $\mathit{d\_clean}$,
$\mathit{d\_unload}$, $\mathit{d\_maintenance}$), enforcing sequential departure
after all ground handling tasks are finished. Runway mutual exclusion is enforced
by the single token in $\mathit{runway}$, which must be consumed by any
transition involving runway use.
...
\begin{itemize}
  \item \textbf{S1} (Runway Mutual Exclusion): No two aircraft shall use the
    ...
    \[
      \mathbf{G}\,(\neg\,\mathit{Runway}_{A1} \wedge \neg\,\mathit{Runway}_{A2}
        \wedge \neg\,\mathit{Runway}_{A3})
    \]

  \item \textbf{S2} (Bay Mutual Exclusion): No more than two aircraft shall
    occupy a bay simultaneously (enforced by $\mathit{bay}$ with initial
    marking~2).
    \[
      \mathbf{G}\,(\neg\,(\mathit{Bay}_{A1} \wedge \mathit{Bay}_{A2}
        \wedge \mathit{Bay}_{A3}))
    \]
  ...
  \item \textbf{S5} (Worker Pool Integrity): Worker pools must never be
    over-committed.
    \[
      \mathbf{G}\,(M(\mathit{maintenance\_team}) \geq 0)
    \]
    and likewise for \textit{clean\_team}, \textit{fuel\_team},
    \textit{catering\_team}, and \textit{unload\_team}, ensuring tokens are
    always returned and the pool count remains non-negative.
\end{itemize}
...
interleavings of concurrent operations across the three aircraft, including all
combinations of fast (A1) and slow (A2, A3) worker allocations.
Table~\ref{tab:statespace} summarises the state space statistics for both the
initial model (before deadlock resolution) and the corrected model (with the
\textit{GSE\_Done} control place).
...
fires $\mathit{sf\_maintenance}$ (consuming all 10 maintenance workers) and A2
subsequently requires the same pool before A1 releases its workers via
$\mathit{ef\_maintenance}$, A2 must wait since $\mathit{ss\_maintenance2}$
requires 2 workers which are unavailable. This resource pressure is correctly
captured in the reachability graph as a non-deadlock scheduling constraint.

\subsection{Deadlock Resolution}

To resolve the identified deadlock, we introduce a \textit{GSE\_Done} control
place with an initial marking of 2. Each aircraft's completion transition
(\textit{all\_done\_1}, \textit{all\_done2}, \textit{all\_done3}) produces a
token to \textit{GSE\_Done}, which is then consumed by the \textit{board}
transition. This bounds the number of aircraft that can complete ground
handling and proceed to boarding concurrently, preventing the circular wait
condition. The approach is analogous to the deadlock avoidance and liveness
control techniques established in~\cite{b4,b5}. After applying this control
mechanism, TINA confirms that no dead markings exist in the reachability graph
other than the intended terminal state where all aircraft have completed takeoff.
...
\begin{figure}[htbp]
  ...
  \caption{Deadlock scenario in the initial model (circular waiting on worker
           pools) and its resolution via the \textit{GSE\_Done} place.}
  \label{fig:deadlock}
\end{figure}
...
confirming that all three aircraft complete their turnarounds under all possible
...
considers three aircraft with apron resource constraints; scaling to larger
fleets would require additional pool competition analysis. Future work will
...
Using the TINA toolbox, we analysed the reachability graph of a three-aircraft
scenario sharing five worker pools with apron resource constraints, detected
deadlock conditions arising from circular worker pool competition, and resolved
them via a \textit{GSE\_Done} control place. All safety and liveness properties were verified in the corrected model.
